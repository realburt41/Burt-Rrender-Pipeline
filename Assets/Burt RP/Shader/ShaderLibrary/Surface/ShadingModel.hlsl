    ...
    float3 UESpecular( float Roughness, float3 SpecularColor,half3 N, half3 L, half3 V)
    {
        float3 H = SafeNormalize(float3(L) + float3(V));
        ...
        // float D  = D_GGX_Mobile(Roughness,NoH);
        float Vis = clamp(Vis_SmithJointApprox( a2, NoV, NoL ),0,100);
        // float Vis = V_SmithGGXCorrelated_Fast(Roughness,NoV, NoL);
